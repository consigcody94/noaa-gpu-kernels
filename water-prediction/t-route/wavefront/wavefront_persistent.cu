/**
 * Persistent (grid-synchronized) wavefront MC with the full secant solver.
 * Single cooperative kernel launch per timestep replaces 1143 per-level
 * launches on the real CONUS topology.
 *
 * Derived from wavefront_mc.cu + the persistent-kernel idea from
 * Gondhalekar et al. 2025 (arXiv:2508.04917).
 *
...
__global__ void kernel_persistent_wavefront(
...
int main(int argc, char** argv) {
    ...
    printf("[io] n_reaches=%d n_segments=%d n_levels=%d\n", t.n_reaches, t.n_segments, t.n_levels);
    ...
    int threads = 256;
    int dev = 0, numSM = 0, maxBlocksPerSM = 0;
    cudaDeviceGetAttribute(&numSM, cudaDevAttrMultiProcessorCount, dev);
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &maxBlocksPerSM, (const void*)kernel_persistent_wavefront, threads, 0);
    int coop_blocks = numSM * maxBlocksPerSM;
    if (coop_blocks <= 0) coop_blocks = numSM;
    printf("[gpu] coop: numSM=%d bpSM=%d -> blocks=%d threads=%d\n",
           numSM, maxBlocksPerSM, coop_blocks, threads);
    ...
    // Warmup
    {
        void* args[] = {&d_params, &d_seg_start, &d_seg_len, &d_up_ptr, &d_up_idx,
                        ...
                        &d_qdc_out, &d_vel_out, &d_dep_out, &dt, &t.n_levels};
        CUDA_CHECK(cudaLaunchCooperativeKernel(
            (void*)kernel_persistent_wavefront,
            dim3(coop_blocks), dim3(threads), args));
    }
    CUDA_CHECK(cudaDeviceSynchronize());

    printf("[gpu] running %d timesteps (persistent wavefront)\n", n_ts);
    cudaEventRecord(e0);
    ...
    printf("[gpu-persistent-secant] total ms: %.3f per-ts: %.3f ms\n", gpu_ms, gpu_ms / n_ts);
    ...
    printf("[gpu-persistent-secant] sum=%.3e max=%.3e nans=%d\n", sum, maxv, nans);
